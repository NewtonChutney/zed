...
use language_model::{
    ...
    LanguageModelRequest, ProviderSettingsView, RateLimiter, SubPageProviderSettings,
};
...
impl LanguageModelProvider for VertexAiLanguageModelProvider {
    ...
    fn authenticate(&self, cx: &mut App) -> Task<Result<(), AuthenticateError>> {
        ...
        cx.spawn(async move |cx| {
            ...
            let access_token = vertex_ai::refresh_access_token(http_client.as_ref(), &credentials)
                .await
                .map_err(|error| AuthenticateError::Other(error.into()))?;

            state.update(cx, |state, cx| {
                let quota_project_id = credentials.quota_project_id.clone();
...
impl LanguageModel for VertexAiLanguageModel {
    ...
    > {
        ...
        let quota_project_id = credentials
            .as_ref()
            .and_then(|c| c.quota_project_id.clone());
